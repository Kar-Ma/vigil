...
- Optional copies saved as ordinary `.mov` files in `iCloud Drive/Vigil/Recordings`
...
- iCloud Drive copying starts only after a recording is finalized and requires iCloud Drive to be enabled. Vigil reports success only after iOS marks the file as uploaded; delayed uploads remain queued while the local Vault copy stays safe.
...
9. Enable **iCloud Drive** in Vigil Settings if you want completed recordings copied to `iCloud Drive/Vigil/Recordings`, where they remain accessible from Files, Finder, Windows, or iCloud.com without Vigil.
10. Leave **Screen Curtain gesture** enabled to hide or reveal the live preview with a three-finger triple-tap. Vigil restores the previous display brightness when the curtain closes or the app leaves the foreground.
11. On a supported iPhone, open **iPhone Settings → Action Button**, choose **Shortcut**, and assign **Start Vigil Recording**. Press and hold the Action Button to open Vigil and begin recording with your default camera mode.
...
- Add durable background retry and clearer upload history for iCloud Drive
